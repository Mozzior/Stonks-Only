<template>
  <div class="h-screen w-screen flex bg-[var(--color-bg-body)]">
    <!-- Left: Brand Section -->
    <div class="hidden lg:flex w-1/2 bg-gradient-to-tr from-[var(--color-brand-secondary)] to-[var(--color-bg-sidebar)] items-center justify-center relative overflow-hidden">
      <div class="absolute top-0 right-0 w-96 h-96 bg-[var(--color-brand-primary)] opacity-10 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2"></div>
      <div class="absolute bottom-0 left-0 w-[500px] h-[500px] bg-purple-600 opacity-10 rounded-full blur-3xl transform -translate-x-1/3 translate-y-1/3"></div>
        ...
        >
          ...
          <div class="mb-6" v-if="model.newPassword">
            <div class="flex justify-between text-xs mb-1">
              <span class="text-[var(--color-text-secondary)]">{{ t('auth.changePassword.strength') }}</span>
              <span :class="strengthColorClass">{{ strengthText }}</span>
            </div>
            <div class="flex gap-1 h-1.5">
              <div class="flex-1 rounded-full transition-colors duration-300" :class="strength >= 1 ? strengthColorClassBg : 'bg-[var(--color-border)]'"></div>
              <div class="flex-1 rounded-full transition-colors duration-300" :class="strength >= 2 ? strengthColorClassBg : 'bg-[var(--color-border)]'"></div>
              <div class="flex-1 rounded-full transition-colors duration-300" :class="strength >= 3 ? strengthColorClassBg : 'bg-[var(--color-border)]'"></div>
            </div>
          </div>

          <n-form-item path="confirmNewPassword" :label="t('auth.changePassword.confirmNewPassword')">
            <n-input
  ...
  confirmNewPassword: [
    ...
    },
    {
      validator: (_rule: FormItemRule, value: string) => {
        return value === model.value.newPassword || new Error(t('auth.changePassword.messages.mismatch'))
      },
      trigger: ['input', 'blur']
    }
  ]
...
function calculateStrength() {
  ...
  if (score < 2) strength.value = 1 // Weak
  else if (score < 4) strength.value = 2 // Medium
