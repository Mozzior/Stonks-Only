<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-[var(--color-text-primary)] mb-1">{{ t('common.welcome', { name: 'Trader' }) }}</h1>
        <p class="text-[var(--color-text-secondary)] text-sm">{{ t('home.subtitle') }}</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <n-card :bordered="false" class="bg-[var(--color-bg-card)] rounded-xl shadow-sm hover:shadow-md transition-shadow group">
        ...
        <div class="mt-2 h-1 w-full bg-[var(--color-border)] rounded-full overflow-hidden">
          <div class="h-full bg-[var(--color-brand-primary)] w-[75%] group-hover:w-[78%] transition-all duration-1000 ease-out"></div>
        </div>
      </n-card>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        <n-card :title="t('home.performance')" :bordered="false" class="bg-[var(--color-bg-card)] rounded-xl min-h-[400px]">
          ...
          <div class="relative w-full h-[320px] bg-gradient-to-t from-[var(--color-brand-primary)]/5 to-transparent rounded-lg border border-dashed border-[var(--color-border)] flex items-end justify-between px-4 pb-4 overflow-hidden group">
             <!-- Simulated Chart Line (SVG) -->
             <svg class="absolute bottom-0 left-0 w-full h-full transition-transform duration-700 group-hover:scale-[1.02]" preserveAspectRatio="none">
               <path d="M0,320 L0,280 C50,250 100,300 150,260 C200,220 250,240 300,200 C350,160 400,180 450,150 C500,120 550,140 600,100 C650,60 700,80 750,50 L800,0 L800,320 Z" fill="url(#grad)" opacity="0.2"></path>
               <path d="M0,280 C50,250 100,300 150,260 C200,220 250,240 300,200 C350,160 400,180 450,150 C500,120 550,140 600,100 C650,60 700,80 750,50" stroke="var(--color-brand-primary)" stroke-width="2" fill="none"></path>
               <defs>
        ...
        <n-card :title="t('home.recentSessions')" :bordered="false" class="bg-[var(--color-bg-card)] rounded-xl">
          <n-data-table
            :columns="columns"
            :data="data"
            :bordered="false"
      ...
      <div class="space-y-6">
        <!-- Quick Actions -->
        <n-card :title="t('home.quickActions')" :bordered="false" class="bg-[var(--color-bg-card)] rounded-xl">
          <div class="grid grid-cols-2 gap-3">
             <n-button class="h-24 flex flex-col gap-2" dashed>
               <n-icon size="24" :component="AddCircleOutline" class="text-[var(--color-brand-primary)]" />
               <span class="text-xs">{{ t('home.actions.deposit') }}</span>
             </n-button>
             <n-button class="h-24 flex flex-col gap-2" dashed>
               <n-icon size="24" :component="ShareOutline" class="text-blue-400" />
               <span class="text-xs">{{ t('home.actions.invite') }}</span>
             </n-button>
             <n-button class="h-24 flex flex-col gap-2" dashed>
               <n-icon size="24" :component="BookOutline" class="text-purple-400" />
               <span class="text-xs">{{ t('home.actions.journal') }}</span>
             </n-button>
             <n-button class="h-24 flex flex-col gap-2" dashed>
               <n-icon size="24" :component="SettingsOutline" class="text-[var(--color-text-secondary)]" />
        ...
        <!-- Market Overview -->
        <n-card :title="t('home.marketMovers')" :bordered="false" class="bg-[var(--color-bg-card)] rounded-xl">
          <n-list>
            <n-list-item v-for="stock in marketMovers" :key="stock.symbol">
              <div class="flex justify-between items-center">
                <div class="flex items-center gap-3">
                  <n-avatar size="small" :style="{ backgroundColor: stock.color }">{{ stock.symbol[0] }}</n-avatar>
                  <div>
                    <div class="font-bold text-[var(--color-text-primary)]">{{ stock.symbol }}</div>
                    <div class="text-xs text-[var(--color-text-secondary)]">{{ stock.name }}</div>
                  </div>
                </div>
                <div class="text-right">
                  <div class="font-medium text-[var(--color-text-primary)]">${{ stock.price }}</div>
                  <div :class="stock.change > 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'" class="text-xs font-bold">
...
const columns = [
  ...
  },
  {
    ...
    key: 'actions',
    render() {
      return h(NButton, { size: 'tiny', ghost: true }, { default: () => 'Review' })
    }
  }
]
...
const marketMovers = [
  { symbol: 'NVDA', name: 'NVIDIA Corp', price: '892.10', change: 4.5, color: '#76B900' },
  { symbol: 'TSLA', name: 'Tesla Inc', price: '175.34', change: -2.1, color: '#E31937' },
  { symbol: 'AAPL', name: 'Apple Inc', price: '168.45', change: 0.8, color: '#A2AAAD' },
  { symbol: 'AMD', name: 'Advanced Micro', price: '180.20', change: 3.2, color: '#000000' },
]
</script>
