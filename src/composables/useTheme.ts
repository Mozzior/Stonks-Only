...
const updateCssVariables = (isDark: boolean) => {
  ...
  if (isDark) {
    ...
    // Add segment color for tabs
    root.style.setProperty("--color-bg-segment", "rgba(255, 255, 255, 0.1)");
    root.style.setProperty(
      "--color-border-segment",
    ...
    // Add segment color for tabs - light mode needs distinct background too
    root.style.setProperty("--color-bg-segment", "#E2E8F0");
    root.style.setProperty("--color-border-segment", "#CBD5E1");
  }
};
...
export function useTheme() {
  ...
  onMounted(() => {
    ...
    const savedCandle = localStorage.getItem(CANDLE_COLOR_KEY) as
      | "standard"
      | "chinese";
    if (savedCandle) {
